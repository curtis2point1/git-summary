...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case fetchCompleteMsg:
		if msg.err != nil {
			m.statusMsg = "Fetch failed: " + msg.err.Error()
			return m, nil
		}
		m.statusMsg = "Fetched " + msg.repoName + ", rescanning..."
	...
	case tea.KeyMsg:
		...
		case "F":
			// Fetch from remote for selected repo
			if m.state == StateReady {
				repo := m.GetSelectedRepo()
				if repo != nil {
					m.statusMsg = "Fetching " + repo.Name + "..."
					return m, fetchRepoCmd(repo.Path, repo.Name)
				}
			}
...
// fetchCompleteMsg is sent when git fetch completes
